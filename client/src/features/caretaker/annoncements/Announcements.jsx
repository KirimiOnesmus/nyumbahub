...
const recipientLabel = (announcement) =>
  announcement.tenantId
    ? (announcement.tenantId.userId?.name ?? "One tenant")
    : "All active tenants";
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4 p-6 border-b border-slate-100">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">
              {announcement.tenantId ? "Direct Message" : "Announcement"}
            </p>
            <h3 className="text-lg font-bold text-slate-900 mt-0.5">
              {announcement.buildingName}
            </h3>
            <p className="text-xs text-slate-500 mt-1 flex items-center gap-1.5">
              {announcement.tenantId ? (
                <LuUserRound className="shrink-0" aria-hidden="true" />
              ) : (
                <LuUsers className="shrink-0" aria-hidden="true" />
              )}
              To: {recipientLabel(announcement)}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <DetailRow
            icon={LuUserCog}
            label="Sent By"
            value={announcement.sentBy?.name ?? "Unknown"}
          />
          <DetailRow
            icon={LuClock}
            label="Sent At"
        ...
        {hasFailures && (
          <div className="p-6 pt-0">
            ...
            >
              <LuRotateCcw
                className={sendingAgain ? "animate-spin" : ""}
                aria-hidden="true"
              />
              {sendingAgain
                ? "Sending…"
                : announcement.tenantId
                  ? `Send Again to ${recipientLabel(announcement)}`
                  : "Send Again to This Building's Active Tenants"}
            </button>
          </div>
        )}
      </div>
...
const Announcements = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const { buildings: buildingList } = await getBuildings({
          page: 1,
          limit: 100,
        });
        const safeBuildings = Array.isArray(buildingList) ? buildingList : [];
        if (!cancelled) setBuildings(safeBuildings);

        const perBuilding = await Promise.all(
          safeBuildings.map((b) =>
            listAnnouncements(b.id, { page: 1, limit: 100 }).catch(() => ({
              announcements: [],
            })),
          ),
        );
        ...
        if (!cancelled)
          setError(
            err.message || "We couldn't load your announcements. Try again.",
          );
      } finally {
  ...
  const filteredAnnouncements = useMemo(() => {
    ...
    return announcements.filter((a) => {
      const matchesSearch =
        ...
        (a.sentBy?.name ?? "").toLowerCase().includes(q) ||
        recipientLabel(a).toLowerCase().includes(q);
  ...
  const handleSendAgain = async (announcement) => {
    ...
    try {
      const { announcement: resent } = announcement.tenantId
        ? await sendDirectAnnouncement(
            announcement.buildingId,
            announcement.tenantId.id,
            announcement.message,
          )
      ...
      // Surfaced implicitly: the modal stays open on the original announcement and the button re-enables, letting the caretaker retry.
  ...
  const columns = useMemo(
    () => [
        ...
        render: (row) => (
          <span className="flex items-center gap-1.5">
            {row.tenantId ? (
              ...
              <LuUsers className="text-slate-400 shrink-0" aria-hidden="true" />
            )}
            {recipientLabel(row)}
          </span>
        ),
      },
      {
        ...
        label: "Message",
        render: (row) => (
          <span className="line-clamp-1 max-w-xs inline-block align-middle">
            {row.message}
          </span>
        ),
      },
