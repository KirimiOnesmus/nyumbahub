...
const Caretakers = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) setError(err.message || 'We couldn\u2019t load your caretakers. Try again.');
      } finally {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {loading && <Loader label="Loading your caretakers…" />}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredCaretakers.map((caretaker) => (
            <CaretakerCard
              key={caretaker.id}
              caretaker={caretaker}
              onOpen={(id) => navigate(`/owner/caretakers/${id}`)}
            />
          ))}
        </div>
      )}
      ...
      {showAddModal && (
        <AssignCaretaker onClose={() => setShowAddModal(false)} onSaved={handleCreated} />
      )}
    </div>
  );
};
