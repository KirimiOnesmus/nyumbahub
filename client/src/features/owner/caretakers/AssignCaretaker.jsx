  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isAssignMode) {
        ...
        onSaved({
          ...caretaker,
          buildings: [...caretaker.buildings, ...assignedBuildings],
        });
      } else {
      ...
      setSubmitError(
        err.message ||
          (isAssignMode
            ? "We couldn't assign the building. Try again."
            : "We couldn't add the caretaker. Try again.")
      );
    } finally {
      setSubmitting(false);
    }
      ...
      {createdTempPassword ? (
        ...
        >
          ...
          <p className="text-sm text-slate-600">
            Share this temporary password with {pendingCaretaker?.name} through a secure,
            out-of-band channel (SMS or WhatsApp call, not email). It won't be shown again.
          </p>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {!buildingsLoading && assignableBuildings.length === 0 && (
                <p className="text-sm text-slate-500 py-1">
                  {isAssignMode
                    ? 'Already assigned to every building.'
                    : 'No buildings yet — you can assign one later.'}
                </p>
              )}
