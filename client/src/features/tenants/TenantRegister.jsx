...
const emptyForm = { fullName: '', phone: '', email: '', idNumber: '' };
const emptyErrors = { fullName: '', phone: '' };

const TenantRegister = () => {
  const { inviteToken } = useParams();

  const [stage, setStage] = useState('validating'); // validating | invalid | form | submitting | success
  ...
  const handleSubmit = async (e) => {
    ...
    setStage('submitting');
    try {
      const result = await onboardTenant(inviteToken, form);
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className={labelClasses} htmlFor="idNumber">
                  ID Number <span className="normal-case font-normal text-slate-400">(optional)</span>
                </label>
                <input
                  id="idNumber"
                  type="text"
