...
import { getOwner, updateOwner } from '../../../services/owner.service.js';
...
const OwnerDetails = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const { owner: res } = await getOwner(id);
        if (!cancelled) setOwner(res);
      } catch (err) {
        if (!cancelled) setError(err.message || "We couldn't load this owner. Try again.");
      } finally {
  ...
  const handleToggleActive = async () => {
    ...
    try {
      const { owner: updated } = await updateOwner(owner.id, { isActive: !owner.isActive });
      setOwner((prev) => ({ ...prev, ...updated }));
    } finally {
  ...
  const startEditing = () => {
    setEditValues({ name: owner.name, email: owner.email ?? '' });
    setEditErrors({});
    setEditError(null);
    setIsEditing(true);
  };
  ...
  const handleSaveEdit = async (e) => {
    ...
    setSavingEdit(true);
    try {
      const { owner: updated } = await updateOwner(owner.id, {
  ...
  const handlePhoneChanged = (newPhone) => {
    setOwner((prev) => ({ ...prev, phone: newPhone }));
    setShowPhoneModal(false);
  };

  const buildings = Array.isArray(owner?.buildings) ? owner.buildings : [];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {loading && <Loader label="Loading owner…" />}
      ...
      {!loading && !error && owner && (
        <>
              ...
              <form onSubmit={handleSaveEdit} className="flex flex-col gap-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input id="edit-phone" type="tel" value={owner.phone} disabled className={`${inputClasses} bg-canvas cursor-not-allowed`} />
                  </div>
          ...
          <div>
            <div className="mb-4">
              <h3 className="text-lg font-bold text-slate-900">Buildings Owned</h3>
              <p className="text-sm text-slate-500 mt-0.5">
                {buildings.length} building{buildings.length === 1 ? '' : 's'} — buildings are created by the
                owner themselves, not the admin.
              </p>
            </div>
            ...
            {buildings.length === 0 ? (
              <Card className="p-0">
                <EmptyState
                  ...
                  description="This owner hasn't added any buildings to their portfolio yet."
                />
              </Card>
            ) : (
      ...
      {showPhoneModal && owner && (
        <ChangePhoneModal person={owner} onClose={() => setShowPhoneModal(false)} onSaved={handlePhoneChanged} />
      )}

      {showResetPasswordModal && owner && (
        <ResetPasswordModal person={owner} onClose={() => setShowResetPasswordModal(false)} />
      )}
    </div>
  );
};
